...
class WeatherApp:
    ...
    def setup_weather_page(self):
        # 城市選擇
        ttk.Label(self.weather_frame, text="選擇城市:").grid(
            row=0, column=0, padx=5, pady=5)
        self.city_var = tk.StringVar()
        self.city_combo = ttk.Combobox(
            self.weather_frame, textvariable=self.city_var)
        ...
        # 行政區選擇
        ttk.Label(self.weather_frame, text="選擇行政區:").grid(
            row=1, column=0, padx=5, pady=5)
        self.district_var = tk.StringVar()
        self.district_combo = ttk.Combobox(
            self.weather_frame, textvariable=self.district_var)
        self.district_combo.grid(row=1, column=1, padx=5, pady=5)

        # 天氣元素選擇
        ttk.Label(self.weather_frame, text="選擇天氣元素:").grid(
            row=2, column=0, padx=5, pady=5)
        self.elements_var = tk.StringVar(value="天氣預報綜合描述")
        self.elements_combo = ttk.Combobox(
            self.weather_frame, textvariable=self.elements_var)
        ...
        # 查詢按鈕
        ttk.Button(self.weather_frame, text="查詢天氣", command=self.get_weather).grid(
    ...
    def setup_earthquake_page(self):
        # 地震類型選擇
        ttk.Label(self.earthquake_frame, text="地震類型:").grid(
            row=0, column=0, padx=5, pady=5)
        self.earthquake_type = tk.StringVar(value="0")
        ttk.Radiobutton(self.earthquake_frame, text="小區域有感地震",
                        variable=self.earthquake_type, value="0").grid(row=0, column=1, padx=5, pady=5)
        ttk.Radiobutton(self.earthquake_frame, text="顯著有感地震", variable=self.earthquake_type, value="1").grid(
            row=0, column=2, padx=5, pady=5)

        # 查詢按鈕
        ttk.Button(self.earthquake_frame, text="查詢地震", command=self.get_earthquake).grid(
            row=1, column=0, columnspan=3, pady=10)

        # 結果顯示區域
        self.earthquake_result = tk.Text(
            self.earthquake_frame, height=20, width=80)
    ...
    def update_districts(self, event=None):
        ...
        if result['status']:
            ...
            if locations:
                districts = [loc["LocationName"]
                             for loc in locations[0].get("Location", [])]
    ...
    def format_earthquake_data(self, data):
        ...
        for eq in eq_list:
            ...
            result.append(
                f"震央位置：{info.get('Epicenter', {}).get('Location', '-')}")
            result.append(
                f"地震規模：{info.get('EarthquakeMagnitude', {}).get('MagnitudeValue', '-')}")
            result.append(f"地震深度：{info.get('FocalDepth', '-')} 公里")
            if max_area:
                result.append(
                    f"最大震度：{max_area.get('CountyName', '-')}，{max_area.get('AreaIntensity', '-')}")
            else:
                result.append("最大震度：-")
